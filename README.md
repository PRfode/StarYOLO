...
MambaYOLO + 轻量化骨干改造（StarNet）

核心逻辑：MambaYOLO将YOLOv8的骨干网络替换为ODMamba，以实现线性复杂度的全局建模能力。在此基础上，引入二次轻量化改进，即在MambaYOLO基础之上，将ODMamba骨干网络的某些部分替换为更轻量的结构（StarNet）
...
## 课程要求
需要使用非最新的论文作为基础项目复现，再在其上使用新论文（2024-2026）的方法进行改进，也就是复现老论文+在老论文的基础上增加新论文的方法进行改进，而且这种改进之前没人做过

## 训练
### 注意：
- 注意修改数据集配置文件中的 `path` 路径。
- 如果使用他人的权重文件，请修改 `args.yaml` 中的路径。
